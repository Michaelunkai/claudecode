...
<#
.SYNOPSIS
    Claude Code + OpenClaw Backup v21.0 - LEAN BLITZ
.DESCRIPTION
    Zero-garbage backup: only useful data, real-time progress, system cleanup.
    v21.0: Excludes 850MB+ of regeneratable caches/binaries vs v20.
...
Write-Host "  CLAUDE CODE + OPENCLAW BACKUP v21.0 LEAN BLITZ" -ForegroundColor White
...
Get-ChildItem $startDir -File -EA SilentlyContinue | Where-Object {$_.Name -match "openclaw|claude|clawd|moltbot|OpenClaw"} | ForEach-Object {
...
if($cmdCache.ContainsKey("schtasks")){
    try{
        $st=$cmdCache["schtasks"]; if($st){
            ...
            if($rel){$rel|ConvertTo-Json -Depth 5|Out-File "$BackupPath\scheduled-tasks\relevant-tasks.json" -Encoding UTF8}
        }
    }catch{}
}
...
Write-Host ""
Write-Host ("=" * 80) -ForegroundColor Cyan
Write-Host "  BACKUP COMPLETE - v21.0 LEAN BLITZ" -ForegroundColor Green
Write-Host ("=" * 80) -ForegroundColor Cyan
