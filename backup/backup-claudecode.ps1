...
function Start-ParallelCopy {
    ...
    $job = Start-Job -ScriptBlock {
        ...
        if (Test-Path $src -PathType Container) {
            ...
            # /E = include empty subdirs, /R:1 = retry once, /W:1 = wait 1 sec, /MT:64 = 8 threads
            # /NFL /NDL /NJH /NJS = suppress file/dir/header/summary logging for speed
            $null = robocopy $src $dst /E /R:0 /W:0 /MT:64 /NFL /NDL /NJH /NJS /XD "node_modules" ".git" "__pycache__" ".venv" "venv" 2>$null
        } else {
...
function Wait-ParallelJobs {
    param([array]$Jobs, [int]$MaxConcurrent = 16, [int]$TimeoutSeconds = 300)
    ...
    while ($Jobs.Count -gt 0) {
        ...
        if ([int](((Get-Date) - $startTime).TotalSeconds) % 10 -eq 0) {
            Write-Step "  -> $($running.Count) jobs running, $($completed.Count) completed..." "INFO"
        }
